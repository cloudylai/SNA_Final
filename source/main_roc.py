        ...
        for (u, v) in L:
            if H.has_edge(u, v):
                ...
                if nx.has_path(H, u, v):
                    X[cnt].append(nx.shortest_path_length(H, source = u, target = v)/50000)
                else:
                    X[cnt].append(1)
                H.add_edge(u, v)
            else:
...
#    clf = rf(n_estimators = 1000, n_jobs = -1, max_depth = 20) #classifier
    ...
    # i = 0: baseline ( g0 )
    # i = 1: +useful graphical features ( g0+g2+g4+g5+g6+c0 )
    # i = 2: +useful check-in features (g0+g2+g4+g5+g6+c0+c1+c3+c5+c6+c7+c8+c9+c10+c11+c12)
    # i = 3: +useful heter-graph features ((g0+g2+g4+g5+g6+c0+c1+c3+c5+c6+c7+c8+c9+c10+c11+c12+h2+h3+h4+h5)
    for i in range(4):
        print("Round",i)
        if i == 0:
            flag['g0'] = True
        ...
        if i == 3:
            ...
            plot_format = "r--"
            plot_name = "g+c+h"
        plot_exp(flag, plot_name, plot_format)
    plt.savefig('exps_ROC.png')
    plt.close()
    '''
    flag['g0'] = True 
    #flag['g1'] = True 
...
def plot_exp(flag, plt_name, plt_format):
    ...
    print ("Write freatures of predicted sample...")
    with open("inspect_false.txt", "w") as f:
        for idx in range(len(L_train)):
            if Y_pred[idx] != Y_train[idx]:
                print(L_train[0], L_train[1], Y_pred[idx], Y_train[idx], X_train[idx], file=f)

    with open("inspect_true.txt", "w") as f:
        for idx in range(len(L_train)):
            if Y_pred[idx] == Y_train[idx]:
                print(L_train[0], L_train[1], Y_pred[idx], Y_train[idx], X_train[idx], file=f)
    '''
...
#    X_test = get_features(L_test, flag)
    ...
    Y_roc_test = []
    for y in Y_test:
        Y_roc_test.append(int(y))
    fpr, tpr, thresholds = roc_curve(Y_roc_test, probas_[:,1], pos_label=1)
    roc_auc = auc(fpr, tpr)
    print ("Area under the ROC curve:", roc_auc)

    #plt.clf()
    plt.plot(fpr, tpr, label='%s ROC curve (area = %0.5f)' % (plt_name, roc_auc))
    plt.plot([0,1], [0,1], plt_format)
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.0])
